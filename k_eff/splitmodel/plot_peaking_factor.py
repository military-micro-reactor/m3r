...
def generate_core_xy_coords():
    ...
    core_basis = np.dot(
        9.7,
        np.concatenate((core_i_hat, core_j_hat)).reshape(2, 2).transpose())

    core_ij = np.array([[0, 1, 1, 2], [0, 0, 1, 0]])
    ...
    for j in range(-4, 1):
        row_ij = np.concatenate(
            (np.arange(imin, imax + 1), j * np.ones(imax - imin + 1))).reshape(
                2, imax - imin + 1).transpose()
    ...
    for assm in range(1, 4):
        for k in range(0, 6):
            rotate_60 = np.array([[cos(k * pi / 3), -sin(k * pi / 3)],
                                  [sin(k * pi / 3),
                                   cos(k * pi / 3)]])
            assm_xy = np.dot(assm_basis, np.array(assm_ij).transpose())
            assm_xy = np.add([[core_disp[0][assm]], [core_disp[1][assm]]],
                             assm_xy)
